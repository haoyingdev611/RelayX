...
export function PayWithSheet(props: Props & NavigationProps<any>) {
  const api = useApi();
  ...
  useEffect(() => {
    const amount = props.amountSAT / currencyExchangeRate / 100000000;
  ...
  const onPaymentUpdate = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.matchSplitOrder(amount, currency, paymentId);
  ...
  const onFiatPress = async () => {
    ...
    if (!paymentId) return;
    setLoading(true);
    const orderRes = await api.acceptSplitOrder(
      sendAmount,
      paymentId,
      orderId,
      props.address
